...
angular rate sensor data, and accelerometer sensor data.

The data packets are sent at a rate
that is internally configured, but can be set by other ECUs.

.. table:: *Data Packet Messages*
    :align: left

    +-------------------+--------+--------+--------------------+--------------------------------+
    | **Data Packet**   | **PF** | **PS** || **Data Length**   |  **Purpose**                   |
    |                   | (hex)  | (dec)  || (bytes)           |                                |
    +-------------------+--------+--------+--------------------+--------------------------------+
    || Slope Sensor     | 240    | 41     | 8                  || Provide high accuracy         |
    || Information      |        |        |                    || pitch & roll rates            |
    |  Type 2           |        |        |                    |                                |
    +-------------------+--------+--------+--------------------+--------------------------------+
    || Angular Rate     | 240    | 42     | 8                  || Provide moderate accuracy     |
    || Sensor Data      |        |        |                    || pitch, roll and yaw rates     |
    +-------------------+--------+--------+--------------------+--------------------------------+
    || Acceleration     | 240    | 45     | 8                  || Provide moderate accuracy     |
    || Sensor Data      |        |        |                    || X, Y, and Z axes acceleration |
    +-------------------+--------+--------+--------------------+--------------------------------+


**Slope Sensor Information - Type 2 (SSI2) Data Packet** - See the *SSI2 Data Packet Layout* table for byte content

.. table:: *SSI2 Data Packet*
    :align: left
...
.. table:: *Angular Rate Data Packet*
    :align: left
...
.. note::   As with all multiple byte fields, the LSB of each of the *Angular Rate Data Packet* fields is set first.

**Acceleration Data Packet**

The following table describes the Acceleration Data Packet:

.. table:: *Acceleration Data Packet*
    :align: left
